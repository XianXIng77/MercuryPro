...
- Routers:
  - `mercury_auth.py` — `/api/auth/*`: email+password login/register (PyJWT + bcrypt). Sessions are JWT HttpOnly cookies (`get_current_user` dependency + `has_valid_session` middleware guard on all `/api/*` except `/api/auth/*` and on `/browser-debug/*`). Users live in `<repo>/data/users.json`; a built-in admin (`m@xianxing.art`) is auto-created on first startup. JWT secret: `MERCURY_JWT_SECRET` env or auto-generated `data/jwt-secret.txt`.
...
- **Registration engine** (the "grok-engine" core):
  ...
  - `chatgpt_registration/` — ChatGPT registration orchestration (`flow`, `worker`, `operations`, `probe`, `history`, `diagnostics`). `diagnostics.py` writes timestamped incident folders under `<repo>/log/openai/` for OpenAI/ChatGPT and `<repo>/log/grok/` for Grok/xAI, each containing `screenshot.png` (when the browser page is alive) and `log.txt` (local timestamp, target, reason, step timeline, and page-text snapshot). Legacy root-level `<repo>/log/<incident>/` folders remain readable. Override the base location with `MERCURY_REGISTRATION_LOG_DIR`.
