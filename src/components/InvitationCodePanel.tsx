...
interface InvitationCodePanelProps { currentPreset: StylePreset; canManage?: boolean; }
...
export const InvitationCodePanel: React.FC<InvitationCodePanelProps> = ({ currentPreset, canManage = true }) => {
...
const copySelected = async () => { if (!selectedCodes.length) return; try { await navigator.clipboard.writeText(selectedCodes.join('\n')); setNotice({ tone: 'ok', text: `已复制 ${selectedCodes.length} 个邀请码，每行一个。` }); } catch { setNotice({ tone: 'error', text: '批量复制失败，请重试。' }); } };
  ...
  return (<>
    ...
    </div>    {revokeTarget && <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/30 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="revoke-dialog-title"><motion.div initial={{ opacity: 0, scale: 0.96, y: 8 }} animate={{ opacity: 1, scale: 1, y: 0 }} className={`w-full max-w-md rounded-2xl border p-5 shadow-2xl ${theme.cardBg} ${theme.border}`}><div className="flex items-start gap-3"><div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-rose-100 text-rose-700 dark:bg-rose-500/15 dark:text-rose-300"><Trash2 className="h-5 w-5" /></div><div className="min-w-0"><h2 id="revoke-dialog-title" className={`text-base font-extrabold ${theme.textPrimary}`}>撤销邀请码</h2><p className={`mt-1 text-xs leading-5 ${theme.textSecondary}`}>确定要撤销 <code className={`font-bold ${theme.textPrimary}`}>{revokeTarget}</code> 吗？撤销后将不能继续注册。</p></div></div><div className="mt-5 flex justify-end gap-2"><button type="button" onClick={() => setRevokeTarget(null)} className={`rounded-xl border px-4 py-2 text-xs font-bold ${theme.border} ${theme.textPrimary}`}>取消</button><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={() => void confirmRevoke()} disabled={!canManage} className="rounded-xl bg-rose-500 px-4 py-2 text-xs font-bold text-white shadow-sm hover:bg-rose-400">确认撤销</motion.button></div></motion.div></div>}
