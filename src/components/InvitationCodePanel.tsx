...
interface InvitationCodePanelProps { currentPreset: StylePreset; canQuery?: boolean; canGenerate?: boolean; canUpdate?: boolean; canDelete?: boolean; canExport?: boolean; }
...
const INVITE_STATUS_OPTIONS: StyledSelectOption[] = [
...
export const InvitationCodePanel: React.FC<InvitationCodePanelProps> = ({ currentPreset, canQuery = true, canGenerate = true, canUpdate = true, canDelete = true, canExport = true }) => {
  ...
  }, [canQuery, keyword, page, pageSize, statusFilter]);

  useEffect(() => { void loadCodes(); }, [loadCodes]);

  const handleLoadCodes = () => { if (!canQuery) { toast.error('无权限'); return; } void loadCodes(); };

  const generateCodes = async () => {
    if (!canGenerate) { toast.error('无权限'); return; }
    setBusy('generate');
  ...
  const confirmRevoke = async () => { if (!canDelete) { toast.error('无权限'); return; } if (!revokeTarget) return; const code = revokeTarget; setRevokeTarget(null); await revokeCode(code); };
...
const copySelected = async () => { if (!selectedCodes.length) return; try { await navigator.clipboard.writeText(selectedCodes.join('\n')); setNotice({ tone: 'ok', text: `已复制 ${selectedCodes.length} 个邀请码，每行一个。` }); } catch { setNotice({ tone: 'error', text: '批量复制失败，请重试。' }); } };
  const exportSelected = async () => { if (!selectedCodes.length) return; if (!canExport) { toast.error('无权限'); return; } try { const result = await grokRegistrationApi.exportInviteCodes(selectedCodes); const blob = new Blob([`${(result.codes || []).join('\n')}\n`], { type: 'text/plain;charset=utf-8' }); const url = URL.createObjectURL(blob); const link = document.createElement('a'); link.href = url; link.download = `invite-codes-${new Date().toISOString().slice(0, 10)}.txt`; link.click(); URL.revokeObjectURL(url); setNotice({ tone: 'ok', text: `已导出 ${(result.codes || []).length} 个邀请码，每行一个。` }); } catch (error) { setNotice({ tone: 'error', text: error instanceof Error ? error.message : '邀请码导出失败' }); } };
  ...
  return (<>
    <div className={`flex h-full min-h-0 flex-1 flex-col overflow-hidden ${theme.appBg}`}>
      <div className="min-h-0 flex-1 overflow-y-auto p-4 sm:p-6"><div className="flex min-h-full w-full flex-col"><div className={`${cardClass} flex min-h-full flex-1 flex-col overflow-hidden`}>
        <div className={`border-b px-5 py-5 sm:px-7 ${theme.border}`}><div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-start gap-3"><div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-blue-600 text-white shadow-sm"><KeyRound className="h-5 w-5" /></div><div className="min-w-0"><h1 className={`text-lg font-extrabold ${theme.textPrimary}`}>邀请码中心</h1><p className="mt-1 text-xs leading-5 text-slate-700">管理工作台用户注册邀请码；AI 注册任务无需邀请码。</p></div></div><div className="grid grid-cols-3 gap-2 sm:min-w-[300px]">{(['valid', 'used', 'invalid'] as InviteStatus[]).map((status) => <motion.div key={status} whileHover={{ y: -2 }} transition={{ type: 'spring', stiffness: 300 }} className={`rounded-xl border px-3 py-2 ${statusMeta[status].className}`}><p className="text-[10px] font-semibold opacity-80">{statusMeta[status].label}</p><p className="mt-0.5 text-lg font-extrabold">{summary[status]}</p></motion.div>)}</div></div></div>
        <div className="flex flex-1 flex-col gap-5 p-5 sm:p-7"><div className={`rounded-2xl border p-5 ${theme.border} ${isDark ? 'bg-blue-500/[0.08]' : 'bg-blue-50/80'}`}><div className="flex items-start gap-3"><ShieldCheck className="mt-0.5 h-5 w-5 shrink-0 text-blue-600 dark:text-blue-300" /><div><h2 className={`text-sm font-bold ${theme.textPrimary}`}>生成注册邀请码</h2><p className={`mt-1 text-xs leading-5 ${theme.textSecondary}`}>邀请码可重复使用；使用过的显示为“已用”，撤销后显示为“无效”。</p></div></div><div className="mt-5 flex flex-col gap-3 sm:flex-row sm:items-end"><label className="space-y-1.5"><span className={`block text-xs font-bold ${theme.textPrimary}`}>生成数量</span><input type="number" disabled={!canGenerate} min={1} max={50} value={count} onChange={(event) => setCount(Math.min(50, Math.max(1, Number(event.target.value) || 1)))} className={`${fieldClass} sm:w-44`} /><span className={`block text-[10px] ${theme.textSecondary}`}>单次最多生成 50 个</span></label><label className="space-y-1.5"><span className="block text-xs font-bold">每个可使用次数</span><input type="number" disabled={!canGenerate} min={1} max={1000} value={maxUses} onChange={(event) => setMaxUses(Math.min(1000, Math.max(1, Number(event.target.value) || 1)))} className="w-full rounded-xl border border-slate-300 bg-white px-3 py-2.5 text-sm" /><span className="block text-[10px] text-slate-500">生成后可单独修改</span></label><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={() => void generateCodes()} disabled={!!busy || loading} className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2.5 text-xs font-bold text-white shadow-sm transition hover:bg-blue-500 disabled:opacity-50"><KeyRound className="h-4 w-4" />{busy === 'generate' ? '生成中…' : '生成邀请码'}</motion.button><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={handleLoadCodes} disabled={!!busy || loading} className={`inline-flex items-center justify-center gap-2 rounded-xl border px-4 py-2.5 text-xs font-bold ${theme.border} ${theme.textPrimary} disabled:opacity-50`}><RefreshCw className={`h-4 w-4 ${loading ? 'animate-spin' : ''}`} />刷新列表</motion.button></div></div>
        <div className={`${cardClass} flex min-h-0 flex-1 flex-col overflow-hidden`}><div className={`flex flex-col gap-3 border-b px-5 py-4 ${theme.border} lg:flex-row lg:items-center lg:justify-between`}><div><h2 className={`text-sm font-bold ${theme.textPrimary}`}>已生成的邀请码</h2><p className={`mt-1 text-[11px] ${theme.textSecondary}`}>支持模糊查询、状态筛选和多选导出。</p></div><div className="flex flex-col gap-2 sm:flex-row sm:items-center"><div className="flex min-w-0 items-center gap-2"><div className="relative min-w-0 flex-1 sm:w-64"><Search className={`pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 ${theme.textSecondary}`} /><input value={keywordInput} onChange={(event) => setKeywordInput(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { setKeyword(keywordInput.trim()); setPage(1); } }} placeholder="模糊搜索邀请码" className={`${fieldClass} pl-9`} /></div><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={() => { if (!canQuery) { toast.error('无权限'); return; } setKeyword(keywordInput.trim()); setPage(1); }} className="rounded-xl bg-blue-600 px-3 py-2.5 text-xs font-bold text-white hover:bg-blue-500">查询</motion.button></div><div className="sm:w-28"><StyledSelect ariaLabel="邀请码状态筛选" value={statusFilter} onChange={(value) => { setStatusFilter(value as 'all' | InviteStatus); setPage(1); }} options={INVITE_STATUS_OPTIONS} isDark={isDark} disabled={!canQuery} /></div></div></div>
          <div className={`flex flex-wrap items-center gap-2 border-b px-5 py-3 ${theme.border}`}><button type="button" onClick={toggleAll} disabled={!codes.length || loading} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-xs font-bold ${theme.border} ${theme.textPrimary} disabled:opacity-40`}>{allSelected ? <Check className="h-3.5 w-3.5" /> : <span className="h-3.5 w-3.5 rounded border border-current" />} {allSelected ? '取消全选' : '选择当前页'}</button><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={() => void copySelected()} disabled={!selectedCodes.length} className="inline-flex items-center gap-1.5 rounded-lg bg-blue-600 px-3 py-1.5 text-xs font-bold text-white disabled:opacity-40"><Copy className="h-3.5 w-3.5" />复制所选 ({selectedCodes.length})</motion.button><motion.button whileTap={{ scale: 0.96 }} type="button" onClick={() => void exportSelected()} disabled={!selectedCodes.length} className="inline-flex items-center gap-1.5 rounded-lg border border-violet-700 bg-violet-700 px-3 py-1.5 text-xs font-bold text-white shadow-sm hover:bg-violet-800 dark:border-violet-400 dark:bg-violet-600 dark:text-white dark:hover:bg-violet-500 disabled:cursor-not-allowed disabled:opacity-60"><Download className="h-3.5 w-3.5" />导出所选</motion.button></div>
          <div className="min-h-0 flex-1 overflow-y-auto">{loading && !codes.length ? <div className={`flex items-center justify-center gap-2 p-16 text-xs ${theme.textSecondary}`}><Loader2 className="h-4 w-4 animate-spin" />正在读取邀请码…</div> : codes.length ? <div className="divide-y divide-slate-500/10">{codes.map((item, index) => { const status = getStatus(item); const selected = selectedCodes.includes(item.code); return <motion.div key={item.code} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.2, delay: Math.min(index * 0.035, 0.25) }} className={`flex flex-col justify-between gap-3 px-5 py-4 transition hover:bg-blue-500/[0.04] sm:flex-row sm:items-center ${selected ? 'bg-blue-500/[0.06]' : ''}`}><div className="flex min-w-0 items-center gap-3"><input type="checkbox" checked={selected} onChange={() => toggleCode(item.code)} className="h-4 w-4 accent-blue-600" /><div className="min-w-0"><code className={`text-sm font-bold tracking-wider ${status === 'invalid' ? 'text-slate-400 line-through' : theme.textPrimary}`}>{item.code}</code><p className={`mt-1 text-[11px] ${theme.textSecondary}`}>生成于 {item.created_at ? new Date(item.created_at).toLocaleString() : '--'} · 已使用 {item.uses} / {getMaxUses(item)} 次{item.last_used_at ? ` · 最近使用 ${new Date(item.last_used_at).toLocaleString()}` : ''}</p></div></div><div className="flex items-center gap-2"><span className={`rounded-full border px-2.5 py-1 text-[10px] font-extrabold ${statusMeta[status].className}`}>{statusMeta[status].label}</span><motion.button whileTap={{ scale: 0.94 }} type="button" onClick={() => void copyCode(item.code)} className={`inline-flex items-center gap-1.5 rounded-lg border px-3 py-1.5 text-[10px] font-bold ${theme.border} ${theme.textPrimary}`}><Copy className="h-3.5 w-3.5" />复制</motion.button><motion.button whileTap={{ scale: 0.94 }} type="button" onClick={() => { if (!canUpdate) { toast.error('无权限'); return; } setEditTarget(item); setEditUses(getMaxUses(item)); }} disabled={!!busy || status === 'invalid'} className="inline-flex items-center gap-1.5 rounded-lg border border-blue-300 px-3 py-1.5 text-[10px] font-bold text-blue-600 disabled:opacity-40"><Pencil className="h-3.5 w-3.5" />次数</motion.button><motion.button whileTap={{ scale: 0.94 }} type="button" onClick={() => { if (!canDelete) { toast.error('无权限'); return; } setRevokeTarget(item.code); }} disabled={!!busy || status === 'invalid'} className="inline-flex items-center gap-1.5 rounded-lg bg-rose-500 px-3 py-1.5 text-[10px] font-bold text-white disabled:opacity-40"><Trash2 className="h-3.5 w-3.5" />撤销</motion.button></div></motion.div>; })}</div> : <div className={`p-16 text-center text-xs ${theme.textSecondary}`}>{keyword || statusFilter !== 'all' ? '没有匹配的邀请码。' : '还没有邀请码，请先生成一个。'}</div>}</div>
