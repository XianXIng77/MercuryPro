    ...
    with ctx._config_lock:
        ...
        if ctx.CONFIG_FILE.exists():
            try:
                ...
                pass
        # Docker services use private DNS names instead of loopback.  These
        # explicit overrides keep an existing persisted desktop config from
        # redirecting traffic back into the application container.
        for config_key, env_key in (
            ("local_solver_url", "PROGROK_SOLVER_URL"),
            ("hotmail_local_base_url", "PROGROK_HOTMAIL_HELPER_URL"),
        ):
            override = str(ctx.os.environ.get(env_key) or "").strip().rstrip("/")
            if override:
                data[config_key] = override
