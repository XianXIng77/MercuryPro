...
# The operator chooses registration concurrency.  The batch's account count is
# the natural ceiling; this value only mirrors the API's technical range.
MAX_CONCURRENCY = int(os.environ.get("GROK2API_REG_MAX_CONCURRENCY", "10000") or 10000)
...
try:
    _GLOBAL_REG_INFLIGHT_MAX = max(
        ...
        min(
            MAX_CONCURRENCY,
            int(
                os.environ.get("GROK2API_REG_GLOBAL_INFLIGHT", str(MAX_CONCURRENCY))
                or MAX_CONCURRENCY
            ),
        ),
    )
except (TypeError, ValueError):
    _GLOBAL_REG_INFLIGHT_MAX = MAX_CONCURRENCY
