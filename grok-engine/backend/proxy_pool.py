...
def _hostport_userpass(raw: str) -> str | None:
    """Normalize the common proxy-provider shorthand formats to a URL."""
    ...
    if "://" in s:
        ...
        if scheme in {"soket5", "socket5"}:
            scheme = "socks5"
    # Support both user:pass@host:port and host:port@user:pass.  The latter is
    # offered by some residential proxy dashboards but is not standard URL
    # syntax, so move its credentials in front of the host before parsing.
    if "@" in rest:
        left, right = rest.rsplit("@", 1)
        left_host, left_sep, left_port = left.rpartition(":")
        right_user, right_sep, right_password = right.partition(":")
        if left_sep and left_host and left_port.isdigit() and right_sep and right_user:
            auth = quote(right_user, safe="")
            if right_password != "":
                auth = f"{auth}:{quote(right_password, safe='')}"
            return f"{scheme}://{auth}@{left_host}:{left_port}"
        if "://" not in s:
            return f"{scheme}://{rest}"
        return f"{scheme}://{rest}" if not s.startswith(f"{scheme}://") else s

    # Already a normal URL without authentication.
    if rest.count(":") <= 1:
        if "://" not in s:
            return f"{scheme}://{rest}"
        return f"{scheme}://{rest}" if not s.startswith(f"{scheme}://") else s
